...
SESSION_NAME="${SESSION_NAME:-hmmwv_v07_crm100_mix25_scratch_training}"
...
if command -v tmux >/dev/null 2>&1; then
  ...
  tmux new-session -d -s "$SESSION_NAME" "cd '$REPO_ROOT' && bash scripts/run_hmmwv_v07_tire_normal_force_omega_300g_crm100_mix25_scratch_training.sh >> '$RUN_LOG_ABS' 2>&1"
  echo "$SESSION_NAME" > "$SESSION_FILE"
  echo "started HMMWV v07 scratch flat/CRM mixed training in tmux session $SESSION_NAME"
  echo "log: $RUN_LOG"
  echo "status: $OUTPUT_DIR/status.json"
