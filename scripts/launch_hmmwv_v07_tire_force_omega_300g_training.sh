...
SESSION_NAME="${SESSION_NAME:-hmmwv_v07_tire_force_omega_300g_training}"
...
if command -v tmux >/dev/null 2>&1; then
  ...
  tmux new-session -d -s "$SESSION_NAME" "cd '$REPO_ROOT' && bash scripts/run_hmmwv_v07_tire_force_omega_300g_training.sh >> '$RUN_LOG_ABS' 2>&1"
  echo "$SESSION_NAME" > "$SESSION_FILE"
  echo "started HMMWV v07 tire-force/omega 300G training in tmux session $SESSION_NAME"
  echo "log: $RUN_LOG"
  echo "status: $OUTPUT_DIR/status.json"
